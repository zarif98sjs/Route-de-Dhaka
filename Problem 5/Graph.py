        ...
        while len(priority_queue) != 0:
            ...
            for j in self.adj[a]:
                ...
                if dist[a] + w < dist[b] and self.typ[(a,b)]==1:  ### this is the main comparison
                    ...
                    tt = self.calculate_time(parent, a,extra_add)
                    now_time = add_minute(first_time , math.floor(tt))
                    if(check(now_time) and dist[b] > dist[a] + w +  get_mod_time(now_time)/60):
                        parent[b] = a  # To track the path
                        dist[b] = dist[a] + w +  get_mod_time(now_time)/60
                        extra_add[(b,a)] =   get_mod_time(now_time)/60
                        extra_add[(a,b)] =    get_mod_time(now_time)/60
                        heapq.heappush(priority_queue, (dist[b], b))
